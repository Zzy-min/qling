# `qingling exports` 顶层本地导出索引规格（2026-05-31）
...
- `qingling exports` 显示最近 10 条本地 Markdown 导出。
- `qingling exports 5` 显示最近 5 条。
- `count` 规则与 `/exports` 一致：非法、小于等于 0 或缺省使用 10；超过 50 截断为 50。
- 支持全局 `--file-state-dir` 配置；推荐放在子命令前，例如 `qingling --file-state-dir <dir> exports 5`。
- 无导出目录时输出空态提示并以 exit code 0 退出。

## 验收

- 单测覆盖 CLI parser 识别 `exports [count]`。
- 单测覆盖帮助文案包含 `qingling exports [count]`。
- smoke 覆盖 `qingling exports 1` 可直接退出、按本地目录输出导出列表且不泄露文件正文。
