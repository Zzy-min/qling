# `qling permissions` 顶层权限状态规格（2026-06-01）
...
- 新增顶层命令 `qling permissions`。
- 新增中文别名 `qling 权限`。
- 输出当前生效的 `guard.permissions.default`。
- 输出权限规则列表的数量、tool pattern、decision、reason。
- 输出环境变量覆盖提示：`QLING_GUARD_PERMISSIONS_DEFAULT`、`QLING_PERMISSIONS_MODE`。
