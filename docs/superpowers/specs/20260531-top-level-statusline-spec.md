# `qingling statusline` 顶层状态线规格（2026-05-31）
...
- `qingling statusline` 输出紧凑状态线并以 exit code 0 退出。
- `qingling 状态线` 等价于 `qingling statusline`。
...
- 单测覆盖 help 文案包含 `qingling statusline` 与 `状态线`。
- smoke 覆盖 `qingling 状态线` 可直接退出并输出本地状态线。
