...
- 输入框只允许一个实时输入提示：`›` 必须位于边框内，不能同时再输出一个裸露的第二 prompt。
- 输入框在等待输入、正在输入和提交后的历史输出中都必须是完整边框；不能只显示上边和左右边，底边不得延迟到 Enter 后才出现。
- 实时输入框输出完整底边后，光标必须回到边框内当前编辑位置，输入体验不能退化为在底边之后编辑。
...
- 窄屏时底部 hints 可以换行，但不应截断到丢失 `/exit`。
