# `qingling privacy` 顶层本地隐私边界报告规格（2026-05-31）
...
- `qingling --help` 展示 `qingling privacy`。
...
- `qingling privacy` 输出“本地数据留存”并以 exit code 0 退出。
...
- 单测覆盖帮助文案包含 `qingling privacy`。
- smoke 覆盖 `qingling privacy` 可直接退出、输出本地留存报告、且不输出会话正文。
