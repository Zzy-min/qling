# 顶层目标控制面 `qingling goal`

## Summary
- 新增顶层本地命令 `qingling goal status|set|clear` 与中文别名 `qingling 目标 ...`。
...
## Public Interface
- `qingling goal`
- `qingling goal status [sessionRef]`
- `qingling goal set <condition>`
- `qingling goal set --session <sessionRef> <condition>`
- `qingling goal clear [sessionRef]`
- `qingling 目标`
- `qingling 目标 状态 [sessionRef]`
- `qingling 目标 设置 <condition>`
- `qingling 目标 清除 [sessionRef]`
...
- `set <condition>` 默认作用于最近保存的 session，并写入 daemon runner 的 active goal，`pending=true`，便于 `qinglingd` 后续推进。
...
- smoke 测试覆盖 `qingling goal status`、`qingling 目标 设置 ...`、`qingling goal clear latest`。
- `npm run ci:check` 通过。
