...
## Summary
- 为 `qling help <topic>` 和 slash 聚焦帮助补充本地静态主题错拼建议。
- 目标是让 `qling help expors`、`/help expors` 这类明显错拼直接指向 `exports`，保持 Claude Code 风格的顺滑纠错。
- 该能力只查询本地静态帮助表，不读取运行时状态、不调用模型、不联网。

## User Journey
- 作为 CLI 用户，我输入 `qling help expors` 时，希望看到 `exports` 的建议和可直接复制的 `qling help exports`。
...
- CLI surface 建议必须包含 `qling help <topic-id>` 和该主题的 CLI usage。
...
## Acceptance
- `formatFocusedHelp("expors", { surface: "cli", binName: "qling" })` 输出建议 `qling help exports` 和 `qling exports [count]`。
- `formatFocusedHelp("导出列", { surface: "cli", binName: "qling" })` 输出建议 `qling help exports` 和 `qling exports [count]`。
