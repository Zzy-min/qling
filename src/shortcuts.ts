export const SHORTCUT_LINES = [
  "",
  "⌨️ 【TUI 快捷键】",
  "-----------------------------------------",
  "界面入口 : /help slash 查看本地命令；/privacy 查看边界；/context 查看上下文；/statusline 查看状态线；Tab agents 打开本地 agents 视图",
  "Enter    : 发送当前输入",
  "Tab      : 空输入时打开本地 /agents mission 视图；slash 前缀补全最佳命令；其他非空输入会保留草稿并提示补全未启用",
  "Shift+Tab: 循环 normal → plan → auto(Always-approve) → normal；原位换框色/角标，保留草稿",
  "Slash    : 输入 / 或 /sk 显示本地命令候选，Tab 补全最佳候选，不自动执行",
  "Ctrl+N   : 插入换行，继续编辑多行 prompt",
  "Ctrl+R   : 用当前输入搜索本会话内历史；未命中会提示并保留草稿",
  "Ctrl+A / Ctrl+E : 移动到输入开头/结尾",
  "Alt+←/→, Alt+B/F : 按词移动；Alt+↑/↓ : 多行输入内按列上下移动（兼容常见 Ctrl+方向键序列）",
  "Ctrl+U / Ctrl+K : 删除光标前/后的输入内容",
  "Delete  : 删除光标后的一个字符",
  "Ctrl+W / Alt+D : 删除光标前/后的一个词（兼容常见 Ctrl+Delete 序列）",
  "Ctrl+L   : 清空当前终端视图并重绘输入栏，不丢弃正在编辑的内容",
  "Ctrl+C   : 非空输入时清空；空输入时再次 Ctrl+C 确认退出",
  "Ctrl+Z   : 恢复最近一次被 Ctrl+C 清空的本地草稿",
  "Ctrl+D   : 空输入时退出；非空输入会提示并保留草稿",
  "/exit, /quit, /q, /退出 : 本地退出 TUI，不调用模型、不写入输入历史",
  "Esc      : 取消未完成的终端控制序列；不提交、不清空输入",
  "Home / End : 移动到输入开头/结尾（兼容常见终端序列）",
  "Paste    : 支持终端 bracketed paste，多行粘贴先进入输入缓冲，不会自动发送",
  "↑ / ↓    : 切换历史输入；回到底部会恢复未发送草稿",
  "← / →    : 移动光标",
  "Ctrl+O   : 切换后续长工具输出的展开/折叠显示",
  "Ctrl+\\   : 打开/关闭会话切换器（↑/↓ · Enter 恢复 · Esc 取消并恢复焦点书签）",
  "Tab(空输入) : 进入 managed scrollback，查看该轮真实用户/助手/工具内容；再 Tab/Space/i 回输入焦点",
  "↑/↓ 或 Shift+↑/↓ : 在 managed scrollback 中按用户轮导航",
  "PgUp/PgDn : 在当前轮真实内容内翻页；Enter/Space/i 回输入",
  "/sessions · /resume : 均打开会话切换器；/resume latest 恢复最近；/sessions list 纯列表",
  "/expand last : 重放并展开最近一次工具输出",
  "焦点模型 : prompt（编辑）| scrollback（浏览）— 对标 Grok AgentView",
  "Tab(空)  : 优先进入轮次浏览；无轮次时仍可 /agents",
  "/queue   : 立即查看本地输入队列状态，不等待当前任务完成",
  "/queue clear, /队列 清空 : 立即清空 pending 输入，不取消正在运行任务",
  "-----------------------------------------",
  "说明: 这些快捷键只作用于本地 TUI 输入缓冲；历史搜索不上传、不持久化。",
  "",
];
